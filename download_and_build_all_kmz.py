...
def create_kmz_file(kml_content, kmz_out_path):
    ...
    os.remove(temp_kml)
    print(f' -> ÉXITO: Creado archivo KMZ en CAPAS/: {kmz_out_path}')

# 5 Fuentes Oficiales de la IDE-Perú (INGEMMET GEOCATMIN)
sources = [
  {
    'name': 'INGEMMET_Cartas_Geologicas_501_100k',
    'url': 'https://geocatmin.ingemmet.gob.pe/arcgis/rest/services/SERV_GEOLOGIA_100K_INTEGRADA/MapServer/0/query?where=1%3D1&outFields=*&f=geojson',
    'out_raw': 'ingemmet_cartas_geologicas_100k.json',
    'out_geojson': 'ingemmet_cartas_geologicas_100k.geojson',
    'out_kmz': 'INGEMMET_Cartas_Geologicas_501_100k.kmz'
  },
  {
    'name': 'INGEMMET_Fallas_Geologicas_1000k',
    'url': 'https://geocatmin.ingemmet.gob.pe/arcgis/rest/services/SERV_GEOLOGIA_FALLAS/MapServer/0/query?where=1%3D1&outFields=*&f=geojson',
    'out_raw': 'ingemmet_fallas_geologicas.json',
    'out_geojson': 'ingemmet_fallas_geologicas.geojson',
    'out_kmz': 'INGEMMET_Fallas_Geologicas_1000k.kmz'
  },
  {
    'name': 'INGEMMET_Peligros_Geologicos_Nacional',
    'url': 'https://geocatmin.ingemmet.gob.pe/arcgis/rest/services/SERV_PELIGROS_GEOLOGICOS/MapServer/0/query?where=1%3D1&outFields=*&f=geojson',
    'out_raw': 'ingemmet_peligros_geologicos.json',
    'out_geojson': 'ingemmet_peligros_geologicos.geojson',
    'out_kmz': 'INGEMMET_Peligros_Geologicos_Nacional.kmz'
  },
  {
    'name': 'INGEMMET_Zonas_Criticas_Peligros',
    'url': 'https://geocatmin.ingemmet.gob.pe/arcgis/rest/services/SERV_PELIGROS_GEOLOGICOS/MapServer/2/query?where=1%3D1&outFields=*&f=geojson',
    'out_raw': 'ingemmet_zonas_criticas.json',
    'out_geojson': 'ingemmet_zonas_criticas.geojson',
    'out_kmz': 'INGEMMET_Zonas_Criticas_Peligros.kmz'
  },
  {
    'name': 'INGEMMET_Catastro_Minero_Vigente',
    'url': 'https://geocatmin.ingemmet.gob.pe/arcgis/rest/services/SERV_CATASTRO_MINERO/MapServer/0/query?where=1%3D1&outFields=*&f=geojson',
    'out_raw': 'ingemmet_catastro_minero.json',
    'out_geojson': 'ingemmet_catastro_minero.geojson',
    'out_kmz': 'INGEMMET_Catastro_Minero_Vigente.kmz'
  }
]

print("=== INICIANDO DESCARGA DE MAPAS Y CONVERSIÓN A KMZ (IDE-PERÚ) ===")

for src in sources:
    print(f"\nDescargando mapa oficial: {src['name']} ...")
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=60) as resp:
            ...
            with open(raw_path, 'wb') as f:
                f.write(data_bytes)
            print(f" -> Guardado dataset crudo en DESCARGAS_MAPAS/: {raw_path} ({len(data_bytes)} bytes)")
            
            geojson_data = json.loads(data_bytes.decode('utf-8'))
            num_feat = len(geojson_data.get('features', []))
            print(f" -> Geometrias recuperadas: {num_feat}")
...
print("\n=== TODAS LAS CAPAS FUERON DESCARGADAS Y CONVERTIDAS A KMZ EN CAPAS/ ===")
